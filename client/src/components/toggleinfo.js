    ...
    <div>
      <Container >
        <ButtonGroup size="medium" color="secondary" aria-label="outlined button group" style={{padding:'10px'}}>
          <Button onClick={setTemp} variant={tempOrSymptoms==='temperature'&&"contained"}>Temperature</Button>
          <Button onClick={setSymptoms} variant={tempOrSymptoms==='symptoms'&&"contained"}>Symptoms</Button>
        </ButtonGroup>
     </Container>
    </div>
  )
